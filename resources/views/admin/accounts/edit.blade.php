...
{{-- ++++++++ title ++++++++ --}}
@section('title')تعديل حساب مالي@endsection
{{-- ++++++++ tab_title ++++++++ --}}
@section('tab_title')تعديل الحسابات المالية@endsection
{{-- ++++++++ header ++++++++ --}}
@section('contentHeader')
تعديل الحسابات المالية&nbsp;<i class="fa  fa-money-bill-wave"></i>
@endsection
{{-- ++++++++ header link ++++++++ --}}
@section('contentHeaderLink')
    <a href="{{ route('admin.accountTypes.create') }}">تعديل الحسابات المالية</a>
@endsection
{{-- ++++++++ active link ++++++++ --}}
      ...
      <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('admin.accounts.update') }}" method="post">
                ...
                <div class="row">
                    {{-- +++++++++++++++ hidden id +++++++++++++++ --}}
                    <input type="hidden" name="account_id" id="account_id" class="form-control" placeholder="ادخل اسم الحساب" value="{{ $data['id'] }}">
                    {{-- ++++++++++++++++++ "name" ++++++++++++++++++ --}}
                    ...
                    <div class="col-md-6  col-sm-6">
                        ...
                        <select name="start_balance_status" id="start_balance_status" class="form-control">
                            <option value="">اختر الحالة</option>
                            <option {{ old('start_balance_status',$data['start_balance_status']) == 1  ? "selected" : "" }} value="1">دائن</option>
                            <option {{ old('start_balance_status',$data['start_balance_status']) == 2  ? "selected" : "" }} value="2">مدين</option>
                            <option {{ old('start_balance_status',$data['start_balance_status']) == 3  ? "selected" : "" }} value="3">متزن</option>
                        </select>
                    ...
                    <div class="col-md-6  col-sm-6">
                        ...
                        <input type="text" name="start_balance" id="start_balance" class="form-control" placeholder="ادخل رصيد اول المدة للحساب" value="{{ old('start_balance',$data['start_balance']) }}" oninput="this.value = this.value.replace(/[^0-9]/g,'');">
                        @error('start_balance')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
