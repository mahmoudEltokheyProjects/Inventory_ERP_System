...
{{-- ++++++++ title ++++++++ --}}
@section('title')اضافة حساب مورد@endsection
{{-- ++++++++ tab_title ++++++++ --}}
   ...
   <div class="col-12">
      <div class="card">
        ...
        <div class="card-header">
            <h3 class="card-title card_title_center">اضافة حساب للمورد</h3>
        </div>
        {{-- ============= card-body ============= --}}
        <div class="card-body">
            <form action="{{ route('admin.suppliers.store') }}" method="post">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6  col-sm-6">
                        ...
                        <input name="name" id="name" class="form-control" placeholder="ادخل اسم المورد" value="{{ old('name') }}">
                        @error('name')
                    ...
                    <div class="col-md-6 col-sm-6">
                        ...
                        <select name="supplier_categories_id" id="supplier_categories_id" class="form-control">
                            ...
                            @if ( !empty($suppliers_categories) )
                                @foreach ($suppliers_categories as $info)
                                    <option {{ old('supplier_categories_id') == $info->id  ? "selected" : "" }} value="{{ $info->id }}">{{ $info->name }}</option>
                                @endforeach
                            @endif
                        </select>
                    ...
                    <div class="col-md-6  col-sm-6">
                        <label>العنوان</label>
                        <input name="address" id="address" class="form-control" placeholder="ادخل عنوان المورد" value="{{ old('address') }}">
                    </div>
                </div> <br />
                {{-- ++++++++++++++++++ "notes" ++++++++++++++++++ --}}
                <div class="row">
